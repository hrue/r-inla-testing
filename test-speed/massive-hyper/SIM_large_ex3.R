...
# data simulation
nsujet=2000 # this number will only increase data size (same number of parameters) ; ideal case 2000
nmarkers=20  # this increases the number of fixed effects and hyperparameters (and data size) ; ideal 20-30
# NHYPER = nmarkers*9+1
# HYPER=c(rep(6.25, nmarkers), 15, rep(c(0.5,0.5,0.5,-0.5,-0.5,-0.5), nmarkers), rep(c(0.5, -0.2), nmarkers))
...
SIM_large2 <- joint(formLong = formLong,
                    ...
                    assoc = c(rep("CV_CS", nmarkers)),
                    control=list(int.strategy="eb", verbose=T, cfg=T, keep = T))
