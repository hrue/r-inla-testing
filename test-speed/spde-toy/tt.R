...
mf <- y ~ 0 + b0 + x + f(idx.u, model=spde, replicate = replicate)
mf <- y ~ 0 + b0 + x + f(idx.u, model=spde, group = replicate,
                         control.group = list(model = "iid"))

dat <- inla.stack.data(stk.e)
dat$replicate <- rep(1:10, each = length(dat$idx.u)/10)
dat$replicate <- sample(1:10, length(dat$idx.u),  replace = TRUE)
res.new <- inla(mf, control.family=pcprec,
                family = "normal", 
                data=dat, ##inla.stack.data(stk.e), ## data 
