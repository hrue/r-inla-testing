...
y <- xx * rnorm(n, sd = 0.1) + x*rnorm(n, sd = 0.1) + rnorm(n, sd = 1) ## use stdnormal as family

LOW <- 0
HIGH <- 1
RATE <- 1
riid <- inla.rgeneric.define(iid.model, n = n, LOW = LOW, HIGH = HIGH, RATE = RATE)
rriid <- inla.rgeneric.define(iid.model, n = n, LOW = LOW, HIGH = HIGH, RATE = RATE)

r <- inla(y ~ -1 + f(idx, x, model = riid) + f(iidx, xx, model = rriid), 
          data = data.frame(y, idx = 1:n, iidx = 1:n, x = x, xx = xx),
          family = "stdnormal", verbose = T, keep = F)
summary(r)
