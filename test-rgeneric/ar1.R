    ...
    "log.prior", "quit"), theta = NULL) 
{
    ...
    interpret.theta <- function() {
        return(list(prec = exp(theta[1L]), rho = 2 * exp(theta[2L])/(1 + 
            exp(theta[2L])) - 1))
    }
    graph <- function() {
    ...
    Q <- function() {
        ...
        a <- 1 + param$rho^2
        b <- -param$rho
        x <- param$prec/(1 - param$rho^2) * c(1, b, rep(c(a, 
            b), n - 2), 1)
        return(x)
    }
    mu <- function() {
...
r = inla(formula, data = data.frame(y, idx), family = "gaussian", num.threads = 1)
