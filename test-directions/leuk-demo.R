...
## Run INLA:
rr  = (inla(formula, family="coxph",
           data = Leuk,
           verbose=TRUE,
           control.inla = list(use.directions = FALSE, h = 0.005),
           control.hazard = list(n.intervals = 75, scale.model = TRUE)
           ))
r  = (inla(formula, family="coxph",
           data = Leuk,
           verbose=TRUE,
           control.inla = list(use.directions = TRUE, h = 0.005),
           control.hazard = list(n.intervals = 75, scale.model = TRUE),
           keep = TRUE
           ))
