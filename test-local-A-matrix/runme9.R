...
kern = dnorm(-h:0, sd= h/2)
kern = kern / sum(kern)
B = inla.as.sparse(my.toeplitz(c(kern[nh],  rep(0,  n-nh),  kern[1:h])))

eta <- 2 + 0.5 * z + 0.5 * zz + 0.5 * zzz + 0.5 * zzzz + as.vector(A %*% x) + as.vector(B %*% xx)
y <- rpois(n, lambda = exp(eta))

AB <- cbind(A, B)
r = inla(y ~ 1 + z + zz + zzz + zzzz + f(idx, w, model="ar1", constr = T, A.local = AB, nrep = 2), 
         data = list(y = y, z = z, zz = zz, zzz = zzz, zzzz = zzzz,
                     idx = 1:n, AB = AB, w = rep(0, n)),
         family = "poisson")
