			   ...
			   inla_cgeneric_data_tp *data, int ny, double *y, int nx, double *x, double *result)
{
	...
	case INLA_CLOGLIKE_LOG_PRIOR:
	{
		double prec[2] = {1, 1};
		ret = Malloc(1, double);
		ret[0] = log(1.0 / sqrt(2.0 * M_PI)) + 0.5 * log(prec[0]) - 0.5 * prec[0] * SQR(theta[0]);
		ret[0] += log(1.0 / sqrt(2.0 * M_PI)) + 0.5 * log(prec[1]) - 0.5 * prec[1] * SQR(theta[1]);
	}
	break;
	...
	case INLA_CLOGLIKE_LOGLIKE:
	{
		...
		for (int i = 0; i < nx; i++) {
			...
			double a = mu * phi;
			double b = -mu * phi + phi;
			result[i] = (a-1.0) * log(y[0]) + (b-1.0) * log(1.0 - y[0]) +
