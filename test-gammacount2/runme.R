...
# Models: log(lambda_i) = x_i' gamma,  where lambda_i = E[N(T_i)] / T_i
# =============================================================
...
# -------------------------------------------------------------
negloglik_gc_rate <- function(par, y, X, T) {
    log_alpha <- par[1]
    gamma <- par[-1]
...
T_obs <- runif(n, 0.333, 3)  # e.g., different durations per unit
...
# ML estimation
# -------------------------------------------------------------
# Initialize with Poisson log-rate (offset = log(T))
glm_init <- glm(y ~ x, family = poisson(), offset = log(T_obs))
